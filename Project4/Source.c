...
void bmpLevelconv(const char* name, const uch* raw_img,
    uint32_t width, uint32_t height, uint16_t bits) {
    ...
    size_t alig = ((width * bits / 8) * 3) % 4;  //bmp 4bit 對齊的公式 size_t alig = ((width*bits/8)*3) % 4;                                           
     for (int j = height - 1; j >= 0; j--) {   // 水平鏡像
         for (unsigned i = width; 0<i; i--) {
             uint32_t idx = j * width + i;
             if (bits == 24) {
                 fwrite((char*)&raw_img[idx * 3 + 2],
                     sizeof(char), sizeof(uch), pFile);
...
void bmpUpsideDown(const char* name, const uch* raw_img,
    uint32_t width, uint32_t height, uint16_t bits) {
...
void Imgraw_upsidedown(const Imgraw* _this, const char* name) {
    const Imgraw* p = _this;
    bmpUpsideDown(name, p->data, p->width, p->height, p->bits);

}

void Imgraw_Levelconv(const Imgraw* _this, const char* name) {
    const Imgraw* p = _this;
    bmpLevelconv(name, p->data, p->width, p->height, p->bits);
}
...
int main() {
    ...
    Imgraw_upsidedown(&img, "upsidedown.bmp");
    Imgraw_Levelconv(&img, "levelconv.bmp");
